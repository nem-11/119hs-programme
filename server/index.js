...
function editor(req, res, next) {
  if (req.user.role === 'viewer') return res.status(403).json({ error: 'Editor required' });
  next();
}
...
app.post('/api/completions', auth, editor, (req, res) => {
  const { date, key, by } = req.body;
  const e = db.getCompletion(date, key);
  if (e) {
    db.deleteCompletion(date, key);
    res.json({ action: 'removed' });
  } else {
    db.addCompletion(date, key, by);
...
app.post('/api/drawings', auth, editor, (req, res) => {
...
app.post('/api/zones/:zoneId/activities', auth, editor, (req, res) => {
...
app.delete('/api/zones/:zoneId/activities/:activityId', auth, editor, (req, res) => {
  db.deleteZoneActivity(Number(req.params.zoneId), Number(req.params.activityId));
  res.json({ ok: true });
});
app.put('/api/zones/:zoneId/activities', auth, editor, (req, res) => {
...
app.delete('/api/zones/:id', auth, editor, (req, res) => {
...
app.get('/api/plan/programme', auth, (req, res) => {
  const wantFull = req.query.full === '1' && req.user.role === 'admin';
...
app.delete('/api/programme-items/:id', auth, editor, (req, res) => {
